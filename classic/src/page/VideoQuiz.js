Ext.define('Player.page.VideoQuiz', {
  ...
  afterComponentLayout: function() {
    ...
    var videoBounds = me.queryById('videoPlayer').getVideoEl().getBoundingClientRect(),
    ...
    if (questionContainer.rendered) {
      questionContainer.setX(videoBounds.left);
      questionContainer.setY(videoBounds.top);
    }
  },
  ...
  */
  ...
  onSubmit: function(event) {
    ...
    // Distractors
    var form = questionContainer.query('form')[0],
    ...
    Ext.Object.each(values, function(key, value, myself) {
      var distractor = me.query('[inputValue="' + value + '"]')[0];
      guess.push({
        letter: value,
        responseText: distractor.boxLabel,
        name: key
      });
    });
    ...
    for (var i = me.questions.length - 1; i >= 0; i--) {
      var question = me.questions[i];
    ...
    if (allAnswered) {
      console.log("Score:", score, me._maxScore, me.quizConfig.passingScore);
      if ((score / me._maxScore) * 100 < me.quizConfig.passingScore) {
        SCORM.SetFailed();
      } else {
        SCORM.SetPassed();
      }
    ...
    if (question.submitResponse) {
      switch (question.qType) {
        ...
        case 'MC':
          ...
          success = SCORM.RecordMultipleChoiceInteraction(question.questionId, scormResponses, correct, scormCorrectResponses, question.questionText.text.replace(/(<([^>]+)>)/ig, ""), question.intWeighting, question.intLatency, question.strLearningObjectiveID);
          break;
        case 'trueFalse':
          success = SCORM.RecordTrueFalseInteraction(question.questionId, responseString, correct, question.answer, question.questionText.text.replace(/(<([^>]+)>)/ig, ""), question.intWeighting, question.intLatency, question.strLearningObjectiveID);
          break;
        case 'fillIn':
        case 'FB':
          success = SCORM.RecordFillInInteraction(question.questionId, responseString, correct, question.answer, question.questionText.text.replace(/(<([^>]+)>)/ig, ""), question.intWeighting, question.intLatency, question.strLearningObjectiveID);
          break;
        default:
